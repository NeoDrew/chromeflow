...
4. **Form submits on social/auth platforms require a real human click.** Reddit, X /
   Twitter, GitHub device-code, mcp.so, and most modern anti-bot-protected sites silently
...
**Codex acts directly** (`click_element` / `fill_input`):
...
- Choices that depend on user preference Codex wasn't told
...
By default the image is returned inline to Codex UNLESS it exceeds ~500KB base64, in
