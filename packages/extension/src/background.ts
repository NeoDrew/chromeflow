  ...
  switch (msg.type) {
    ...
    case "click_element": {
      ...
      if (hasUntil) {
        ...
        // X / GitHub device flow without needing a specific until_* destination.
        const start = Date.now();
        ...
        if (!untilResult) {
          untilResult = {
            ok: false,
            reason: `submit silently rejected (likely anti-bot): no URL change, toast, alert, or modal appeared within 4s. Synthetic clicks fail on Reddit / X / GitHub device flow / mcp.so even though isTrusted passes — pre-fill the form, then highlight + wait_for_click so a real human gesture fires the submit.`,
          };
        }
      } else {
